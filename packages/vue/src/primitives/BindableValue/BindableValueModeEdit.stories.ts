...
import BindableValueRoot from './BindableValueRoot.vue'

// Kept apart from BindableValue.stories.ts: this fixture builds a real editor, and the
// docs pages compose the lighter state matrix, so the engine must not share its module.
const meta = {
...
export const ModeEditing: Story = {
  render: () => ({
    ...
    setup() {
      ...
      const targets = ref(
        [first, first, second].map((node) => ({ nodeId: node.id, path: 'width' }))
      )
